...
list_mark=sorted(list_mark)
print(list_mark)
list_locx=[]
list_locy=[]
# Get values paramerter location from Mark
total_mark = len (list_mark)
for k in range(total_mark):
    for j in foundation:
        if list_mark[k]== j.LookupParameter("Mark").AsString():
            point_Units_X = UnitUtils.ConvertFromInternalUnits(j.Location.Point.X, DisplayUnitType.DUT_METERS)
            point_Units_Y = UnitUtils.ConvertFromInternalUnits(j.Location.Point.Y, DisplayUnitType.DUT_METERS)
            list_locx.append(point_Units_X)
            list_locy.append(point_Units_Y)     
# writer csv, export csv  
